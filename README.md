...
### V 1.2.1版本
2026.05.28
优化告警文案内容，当前在配置用户手机号的情况下，会在钉钉@对应人员   感谢@nuoyan
提交漏洞关联资产查询优化  当前显示格式为：资产名称（URL）   感谢@nuoyan
优化资产列表后端无序导致前端显示隔页重复的bug   感谢@nuoyan
复测/修复  默认不再是通过且增加二次确认防止手滑    感谢@nuoyan

---------------------------------------------------------------------
### V 1.3 版本（重大更新，数据库需重构）
2026.06.13
出于某些原因，资产增加地理位置字段，soc系列联动
增加LDAP认证支持，支持本地用户/LDAP用户双模式支持 感谢@Louisnie @meetdoctor @DreamDoctor
```
---
### 临时拉了个群，对SOC/SDL感兴趣的师傅欢迎来玩
![img.png](imgs/wxq.png)